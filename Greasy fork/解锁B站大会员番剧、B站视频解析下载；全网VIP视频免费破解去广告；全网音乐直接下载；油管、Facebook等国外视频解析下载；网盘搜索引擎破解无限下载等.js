			...
			setInterval(function(){
				...
				if(host.indexOf("youtube.com")!=-1){
					if(href.indexOf("youtube.com/watch")!=-1){
						...
						var html='<div id="'+eleId+'" style="width:25px;padding:10px 0px;text-align:center;background-color:#E5212E;position:fixed;top:250px;left:0px;color:#FFF;font-size:0px;cursor:pointer;margin:0px auto;text-align:center;">'+
	...
	try{
		if(functionController.superVideoHelper){
			const superVideoHelperObject = new superVideoHelper(newOriginalInterfaceList),
				superVideoHelperMobileObject = new superVideoHelperMobile(newOriginalInterfaceList);
			const isRunSuperVideoHelper = superVideoHelperObject.isRun(),
				isRunSuperVideoHelperMobile = superVideoHelperMobileObject.isRun();
			if(isRunSuperVideoHelper || isRunSuperVideoHelperMobile){
				if(commonFunctionObject.GMgetValue("copyright_video_remind",null)==="true"){
					if(isRunSuperVideoHelper) superVideoHelperObject.start();
					if(isRunSuperVideoHelperMobile) superVideoHelperMobileObject.start();
				}else{
					var r=confirm(
						"提醒！！！\u000d"+
						"1、VIP视频解析中所用到的解析接口全部收集自互联网，版权问题请联系相关解析接口所有者，脚本不承担相关责任！\u000d"+
						"2、为创造良好的创作氛围，请大家支持正版！\u000d"+
						"3、脚本仅限个人学习交流，同意使用即已代表您已经充分了解相关问题，否则后果自负，特此声明！\u000d"+
						"4、此提醒只弹出一次，确认后，后续将不在弹出，请知悉！"
					);
					if(r==true){
